...
return [
    ...
    'faq_a4' => '閲覧制限に達すると、暗号化されたコンテンツはサーバー上で完全に破棄されます。メタデータのみ（作成日、閲覧回数）がシークレットの存在と消費の証拠として保持されます。たとえリンクとキーが傍受されても、シークレットはもう存在しません — 復号するものは何もありません。',
    ...
    'faq_a12' => '作成時にメールアドレスを入力した場合、「管理」リンクからシークレットを管理（取り消し、延長）できます。メールアドレスを入力すると、10分間有効な使い捨てリンクが届きます。このマジックリンクは意図的なセキュリティ選択です。パスワードがないため、盗まれたり、漏洩したり、総当たり攻撃される対象がありません。メールアドレスが唯一の認証要素となります — シンプルで安全です。',
    ...
    'sbd_zk_title' => 'ゼロナレッジ：約束ではなく数学的不可能性',
    'sbd_zk_desc' => 'サーバーは暗号化キーを受け取りません。データベースが侵害されても、攻撃者が得るのは無意味な暗号文だけです。これはプライバシーポリシーではなく、数学的な不可能性です。',
    'sbd_ml_title' => 'マジックリンク：パスワード攻撃ベクトルの排除',
    'sbd_ml_desc' => 'パスワードがないということは、盗む、漏洩する、総当たりする対象がないということです。メールアドレスが唯一の認証要素です。侵害されたデータベースから認証情報は漏れません — そもそも存在しないからです。',
    'sbd_fragment_title' => 'URLフラグメント：キーはサーバーに触れない',
    'sbd_fragment_desc' => '復号キーはURLフラグメント（#）に存在します。HTTPプロトコルの設計上、この部分はサーバーに送信されません — リクエストにもログにもどこにも。',
    'sbd_destroy_title' => '自動破棄：露出の最小化',
    'sbd_destroy_desc' => '読まれたシークレットはもう存在しません。設定可能な有効期限と使い捨てオプションにより、露出時間を最小限に抑えます。',
